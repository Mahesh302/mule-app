...
if [ ! -d mule-app ]; then
  echo "Cloning mule-app repo..."
  git clone git@github.com:Mahesh302/mule-app.git
else
  echo "mule-app directory exists. Skipping clone."
fi
...
if [[ "$BRANCH_NAME" == feature/* ]]; then
  ...
  # Merge into main and run Postman tests
  git checkout main
  git pull origin main
  git merge origin/release --no-ff --allow-unrelated-histories -m "Auto-merge from release"
  ...
  echo "Branch is not a feature branch. Skipping full flow."
fi
